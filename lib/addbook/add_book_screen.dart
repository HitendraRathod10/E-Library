...
class _AddBookScreenState extends State<AddBookScreen> {
  ...
  void selectImage(context) async{
    ...
    File compressImage = await Provider.of<LoginProvider>(context,listen: false).imageSizeCompress(image: File(filePath!));
    setState((){
      file = compressImage;
  ...
  void uploadFile(context) async {
    ...
    try {
      ...
      checkBoxValue == true ? authorUploadFile(imageUrl) : '';
  ...
  void authorUploadFile(String imageUrl) async {
    if (authorImageFile == null) return;
    final authorImageDestination = 'author/$authorImageName';
    try {
      final authorImageRef = FirebaseStorage.instance.ref().child(authorImageDestination);
      UploadTask authorUploadsTask =  authorImageRef.putFile(authorImageFile!);
      final snapshot3 = await authorUploadsTask.whenComplete(() {});
      final authorUrl = await snapshot3.ref.getDownloadURL().whenComplete(() {});

      checkBoxValue == true ? AddBookProvider().addAuthorDetails(
          authorName: authorNameController.text.trim(),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFieldMixin().textFieldWidget(
                        ...
                        readOnly: true,
                        controller: publishDateController..text = DateTime.now().toString().substring(0,10),
                        border: InputBorder.none,
                        validator: (value) {
                          if (value == null ||
                              value.isEmpty ||
                          ...
                          ClipRRect(
                            ...
                            child: GestureDetector(
                                ...
                                Container(
                                    ...
                                    height: 80,width: 120,
                                    child: const Center(child: Text('Select Image',style: TextStyle(color: AppColor.whiteColor)))) :
                                Image.file(
                                  file!,
                                  height: 80,width: 120,
                                  fit: BoxFit.fill,)
                          ...
                          ClipRRect(
                            ...
                            child: GestureDetector(
                                ...
                                Container(
                                    color: AppColor.darkGreyColor.withOpacity(0.3),
                                    height: 80,width: 120,
                                    child: const Center(child: Text('Select Book',style: TextStyle(color: AppColor.whiteColor)))) :
                                Container(
                                    color: AppColor.darkGreyColor.withOpacity(0.3),
                                    height: 80,width: 120,
                                    child:  Center(child: Text(bookFileName,textAlign: TextAlign.center,style: const TextStyle(color: AppColor.darkGreen))))
                            ),
                          ),
                        ],
                      ),
                                ...
                                Column(
                                  children: [
                                    Container(
                                      ...
                                      child: const Icon(Icons.person_add_alt_1_sharp,size: 50,color: AppColor.whiteColor,),
                                    ),
